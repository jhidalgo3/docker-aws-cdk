...
Run as a command using `cdk` as entrypoint:
    ...
    docker run --rm --entrypoint cdk jhidalgo3/aws-cdk --version

Run as a shell and mount `.aws` folder and current directory as volumes:

    docker run --rm -it -v ~/.aws:/root/.aws -v $(pwd):/opt/app jhidalgo3/aws-cdk bash

Using docker-compose:

    cdk:
        image: jhidalgo3/aws-cdk
        env_file: .env
        entrypoint: aws
        working_dir: /opt/app
        volumes:
...
- [docker-aws-cdk](https://github.com/contino/docker-aws-cdk)
